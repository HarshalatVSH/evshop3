...
const syncBadge = (tabId, context, user) => {
  ...
  if (context?.brand?.active && notif) {
    ...
    browser.action.setIcon({ tabId, path: ACTIVE_ICONS });
    browser.action.setBadgeText({ tabId, text: "1" });
    ...
    browser.action.setIcon({ tabId, path: INACTIVE_ICONS });
...
browser.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  switch (msg.type || "") {
    ...
    case MessageType.CONTEXT:
      ...
      Promise.all([loadContext(msg.data), loadUser()]).then(
        ([context, user]) => {
...
const permissionsToRequest = {
  permissions: ["cookies"],
  origins: ["https://expertvoice.com/*"],
};

async function requestPermissions(tab) {
  try {
    ...
    // TODO: ---------- START BACKGROUND SCRIPT HERE ----------
  } catch (error) {
